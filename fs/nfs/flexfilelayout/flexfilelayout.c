			...
			struct nfs_pgio_header *hdr)
{
	...
	ret = nfs_initiate_pgio(desc, ds->ds_clp, ds_clnt, hdr, ds_cred,
				ds->ds_clp->rpc_ops,
				vers == 3 ? &ff_layout_read_call_ops_v3 :
					    &ff_layout_read_call_ops_v4,
				0, RPC_TASK_SOFTCONN, localread);
	put_rpccred(ds_cred);
	return ret == 0 ? PNFS_ATTEMPTED :
			  (localread ? PNFS_TRY_AGAIN : PNFS_NOT_ATTEMPTED);
			 ...
			 struct nfs_pgio_header *hdr, int sync)
{
	...
	int vers;
	struct nfs_fh *fh;
	int idx = hdr->pgio_mirror_idx, ret;
	...
	put_rpccred(ds_cred);
	return ret == 0 ? PNFS_ATTEMPTED :
			  (localwrite ? PNFS_TRY_AGAIN : PNFS_NOT_ATTEMPTED);
}
...
static int ff_layout_initiate_commit(struct nfs_commit_data *data, int how)
{
	...
	ret = nfs_initiate_commit(ds->ds_clp, ds_clnt, data,
				  ds->ds_clp->rpc_ops,
